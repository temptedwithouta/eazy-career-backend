...
export default class AuthTokenAuthMiddleware extends IsAuthMiddleware {
  protected specificValidation = async (req: Request, res: Response): Promise<void> => {
    ...
    if (!new RegExp(/^auth\-[0-9]+$/).test(authorization.payload.jti)) {
      throw new ClientError(
        401,
        "Request not valid",
        new ClientErrors().setHeadersErrors({
    ...
    const dbResponseSession: Session = await this.checkSession(sessionId, "USER_AUTH");
